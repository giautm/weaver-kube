...
WORKDIR /weaver/
RUN GOPATH=/weaver/ go install {{range .}} {{.}}{{end}}
{{end}}
FROM gcr.io/distroless/static-debian11
WORKDIR /weaver/
{{if . }}
COPY --from=builder /weaver/bin/* /weaver/
{{end}}
COPY . .
...
func buildImage(ctx context.Context, specs *imageSpecs) error {
	...
	return dockerBuild(ctx, workDir, filepath.Join(workDir, dockerfileTmpl.Name()), specs.name)
}

// Use docker-cli to build the docker image.
func dockerBuild(ctx context.Context, buildContext, fileName, tag string) error {
	c := exec.CommandContext(ctx, "docker", "build", buildContext, "-f", fileName, "-t", tag)
	c.Stdout = os.Stdout
	c.Stderr = os.Stderr
	return c.Run()
}
